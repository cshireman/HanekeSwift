...
public class Cache<T : DataConvertible where T.Result == T, T : DataRepresentable> {
    ...
    lazy var cachePath : String = {
        ...
        let cachePath = NSURL(fileURLWithPath: basePath).URLByAppendingPathComponent(self.name).absoluteString
        return cachePath
    }()
    
    func formatPath(formatName formatName : String) -> String {
        let cacheURL = NSURL(fileURLWithPath: self.cachePath)
        let formatPath = cacheURL.URLByAppendingPathComponent(formatName).absoluteString
