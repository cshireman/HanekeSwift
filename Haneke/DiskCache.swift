...
public class DiskCache {
    ...
    public class func basePath() -> String {
        let cachesPath = NSURL(fileURLWithPath:NSSearchPathForDirectoriesInDomains(NSSearchPathDirectory.CachesDirectory, NSSearchPathDomainMask.UserDomainMask, true)[0])
        let hanekePathComponent = HanekeGlobals.Domain
        let basePath = cachesPath.URLByAppendingPathComponent(hanekePathComponent)
    ...
    public lazy var cacheQueue : dispatch_queue_t = {
        let queueName = HanekeGlobals.Domain + "." + NSURL(fileURLWithPath: self.path).lastPathComponent!
        ...
        dispatch_async(cacheQueue, {
            ...
            do {
                ...
                for pathComponent in contents {
                    ...
                    let path = cacheURL.URLByAppendingPathComponent(pathComponent).absoluteString
                    do {
    ...
    public func pathForKey(key : String) -> String {
        ...
        let keyURL = NSURL(fileURLWithPath: self.path)
        let keyPath = keyURL.URLByAppendingPathComponent(filename).absoluteString
        return keyPath
    }
